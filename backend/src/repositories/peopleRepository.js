...
async function create({ identifierType, identifierValue, verdict, approvalExpiration }) {
  const { rows } = await db.query(
...
async function update(id, { identifierType, identifierValue, verdict, approvalExpiration }) {
  const { rows } = await db.query(
    `UPDATE people
  ...
  try {
    ...
    for (const r of records) {
      const { rows } = await client.query(
        ...
        [r.identifierType, r.identifierValue, r.verdict, r.approvalExpiration || null]
      );
